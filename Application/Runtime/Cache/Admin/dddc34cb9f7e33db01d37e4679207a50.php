...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top" id="mainNav">
    ...
    <div class="content-wrapper">
        <div class="container">
            <div class="page-header">
                <h1>修改<?php echo ($teacherinfo['name']); ?>老师的信息</h1>
            </div>
            <div class="row">
                ...
                <div class="col-md-6 col-md-offset-3">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="form-control" value="<?php echo ($teacherinfo['name']); ?>">
                    </div>
                    <div class="form-group">
                        <label for="password" class="control-label">新的密码:</label>
                        <input type="text" name="password" id="password" class="form-control" placeholder="留空则保留原来密码!">
                    </div>
                    <div class="form-group">
                        <label for="number" class="control-label">教工号:</label>
                        <input type="text" name="number" id="number" class="form-control" value="<?php echo ($teacherinfo['number']); ?>">
                    </div>
                    <div class="form-group">
                        <label for="sex" class="control-label">性别:</label>
                        <select name="sex" id="sex" class="form-control">
                            <option value=""></option>
                            <option value="0" <?php if($teacherinfo['sex'] == 0): ?>selected<?php endif; ?> >男</option>
                            <option value="1"  <?php if($teacherinfo['sex'] == 1): ?>selected<?php endif; ?> >女</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="zhicheng" class="control-label">职称:(多个以英文逗号隔开)</label>
                        <input type="text" name="zhicheng" id="zhicheng" class="form-control" value="<?php echo ($teacherinfo['zhicheng']); ?>">
                    </div>
                    <div class="form-group">
                        <label for="job" class="control-label">职务:</label>
                        <input type="text" name="job" id="job" class="form-control" value="<?php echo ($teacherinfo['job']); ?>">
                    </div>
                    <button class="btn btn-primary" onclick="javascript:modify()"><span class="glyphicon glyphicon-plane"></span>确认修改</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function modify() {
            ...
            $.ajax({
                type: "POST",
                url: "/Admin/Teacher/ajax_editTeacher",
                data: {
                    id: <?php echo ($teacherinfo['id']); ?>,
                    name:name,
                    password:password,
                    number:number,
                    sex:sex,
